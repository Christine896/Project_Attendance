...
router.get('/stats/:studentId', verifyToken, async (req, res) => {
    try {
        ...
        const stats = await Promise.all(courseUnits.map(async (unit) => {
            ...
            const actualTotalSessions = await ClassSession.countDocuments({ unitCode: unit.code });
            const percent = actualTotalSessions > 0 ? (attendedCount / actualTotalSessions) * 100 : 100;

            // Fetch last notification for this student/unit combo
            ...
            if (actualTotalSessions >= 3) {
                if (percent < 75) {
                    // WARNING: Send if the last message wasn't already a warning
                    if (!lastNotif || lastNotif.type === 'success') {
                        await Notification.create({ 
                            student: student._id, 
                            unitCode: unit.code, 
                            title: 'Attendance Warning', 
                            message: `Your attendance for ${unit.name} is at ${Math.round(percent)}%. (Requirement: 75%)`, 
                            type: 'warning' 
                        });
                    }
                } else {
                    // RECOVERY (>= 75%): Send if they were previously in a warning state
                    if (lastNotif && lastNotif.type === 'warning') {
                        await Notification.create({ 
                            student: student._id, 
                            unitCode: unit.code, 
                            title: 'Attendance Recovered', 
                            message: `Great job! Your attendance for ${unit.name} is back up to ${Math.round(percent)}%.`, 
                            type: 'success' 
                        });
                    }
                }
            ...
            return { unitId: unit._id, unitCode: unit.code, unitName: unit.name, totalSessions: actualTotalSessions, attended: attendedCount };
        }));
